...
namespace HomeApi.Controllers
{
    ...
    public class RoomsController : ControllerBase
    {
        ...
        public RoomsController(IRoomRepository repository, IMapper mapper, IDeviceRepository devices)
        {
            ...
            [FromBody] EditRoomRequest request)
        {
            ...
            if (oldName != room.Name)
                await _devices.UpdateDeviceRoom(room);

            return StatusCode(200, $"Комната обновлена! Имя - {room.Name}, " +
                                   $"Площадь - {room.Area},  " +
                                   $"Газ подключен - {room.GasConnected}, " +
                                   $"Напряжение - {room.Voltage}"
            );
        }
